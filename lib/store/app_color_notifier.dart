...
class AppColorNotifer extends _$AppColorNotifer {
  @override
  AppColor build() {
    final appColor = ref.watch(preferenceNotifierProvider(Preference.appColor));
    // final nullableValue = ref
    //     .watch(preferenceNotifierProvider<bool?>(Preference.nullableBool));
    return AppColor.fromName(appColor);
  }

  Future<void> update(AppColor appColor) async {
    await ref
        .read(preferenceNotifierProvider(Preference.appColor).notifier)
        .update(appColor.name);
    // await ref
    //     .read(
    //       preferenceNotifierProvider<bool?>(Preference.nullableBool)
    //           .notifier,
    //     )
    //     .update(null);
    ref.invalidateSelf();
